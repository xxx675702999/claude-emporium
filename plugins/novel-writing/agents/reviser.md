...
1. Parse LLM response sections: `FIXED_ISSUES`, `REVISED_CONTENT` (or `PATCHES` for spot-fix), `UPDATED_STATE`, `UPDATED_LEDGER` (if numerical genre), `UPDATED_HOOKS`
2. For spot-fix mode: apply patches to original chapter content
3. For governed mode: merge updated hooks with full hook pool (preserving unmentioned hooks)
...
```json
{
  ...
  "updatedState": "string (updated current_state markdown)",
  "updatedLedger": "string (updated resource_ledger markdown, empty if no numerical system)",
  "updatedHooks": "string (updated pending_hooks markdown)",
  "tokenUsage": {
    "promptTokens": 8500,
...
- Return revised content + updated state/hooks
...
- Chapter review cycle (applies revised content, triggers re-audit)
- Truth file persistence (receives updatedState, updatedLedger, updatedHooks)
